...
export default class ChangeLabel extends React.Component {
    ...
    handleSubmit = async event => {
        ...
        if(this.state.newNameLabel !== "") {
            console.warn('Змінити мітку: ' + this.props.element.label + " на " + this.state.newNameLabel);
            await axios.post(`/change-label`, {
    ...
    render() {
        return (
            <form className={classes.change_item_form} name="form-change-label" onSubmit={this.handleSubmit}>
                <input className={classes.input_name} type="text" value={this.state.newNameLabel} onChange={this.handleChange} /><br/>
                <input className={classes.button_accept} type="submit" value="Змінити" />
                <input className={classes.button_cancel} type="reset" onClick={this.props.showChangeLabelForm} value="Скасувати" />
            </form>
        );
    }
}