...
export default class ChangeTask extends React.Component {
    ...
    handleSubmit = async event => {
        ...
        if(this.state.newNameTask !== "") {
            console.warn('Змінити task: ' + this.props.element.task + " на " + this.state.newNameTask);
            await axios.post(`/change-task`, {
    ...
    render() {
        return (
            <form className={classes.change_item_form} name="form-change-task" onSubmit={this.handleSubmit}>
                <input className={classes.input_name} type="text" value={this.state.newNameTask} onChange={this.handleChange} /><br/>
                <input className={classes.button_accept} type="submit" value="Змінити" />
                <input className={classes.button_cancel} type="reset" onClick={this.props.showChangeTaskForm} value="Скасувати" />
            </form>
        );
    }
}